...
{
    return TryAnalyze(() => ProfilerTraceAnalyzer.AnalyzeSpeedscope(speedscopePath), "Speedscope parse failed");
}

CpuProfileResult? AnalyzeCpuTrace(string traceFile)
{
    return TryAnalyze(() => traceAnalyzer.AnalyzeCpuTrace(traceFile), "CPU trace parse failed");
}
...
{
    return TryAnalyze(() => traceAnalyzer.AnalyzeExceptionTrace(traceFile), "Exception trace parse failed");
}
...
{
    return TryAnalyze(() => traceAnalyzer.AnalyzeContentionTrace(traceFile), "Contention trace parse failed");
}

AllocationCallTreeResult? AnalyzeAllocationTrace(string traceFile)
{
    return TryAnalyze(() => traceAnalyzer.AnalyzeAllocationTrace(traceFile), "Allocation trace parse failed");
}

TResult? TryAnalyze<TResult>(Func<TResult> analyze, string failureMessage)
    where TResult : class
{
    try
    {
    ...
    {
        AnsiConsole.MarkupLine($"[{theme.AccentColor}]{failureMessage}:[/] {Markup.Escape(ex.Message)}");
        return null;
    }
}
