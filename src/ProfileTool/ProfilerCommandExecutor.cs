...
internal sealed class ProfilerCommandExecutor
{
    ...
    private ProfilerJitModeExecutor _jitModeExecutor = null!;
    ...
    public void Execute(ProfilerCommandInvocation invocation)
    {
        ...
        if (_jitModeExecutor.TryHandle(request))
        {
            return;
        }

        if (!_jitModeExecutor.ValidateHotJitRequest(request))
        {
            return;
        }
    ...
    private void RefreshThemeServices()
    {
        ...
        _jitModeExecutor = new ProfilerJitModeExecutor(() => _theme, _jitCommandRunner, _jitOutputFormatter, WriteOutputFiles);
    }
