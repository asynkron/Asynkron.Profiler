    ...
    public void Print(ExceptionProfileResult? results, ProfileRenderRequest request)
    {
        ...
        if (summaryThrown > 0)
        {
            AnsiConsole.Write(_callTreeRenderer.BuildExceptionCallTree(new ProfilerExceptionCallTreeRequest(
                ...
                ProfileRenderRequestHelpers.BuildTreeRootSelectionOptions(request))));
        }
        ...
        {
            var filteredCatchSites = catchSites.Where(entry => ProfileRenderRequestHelpers.MatchesFunctionFilter(entry.Name, request.FunctionFilter));
