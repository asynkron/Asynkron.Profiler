...
internal sealed class ExceptionConsoleProfileRenderer
{
    private readonly Theme _theme;
    private readonly ProfilerCallTreeRenderer _callTreeRenderer;

    public ExceptionConsoleProfileRenderer(
        Theme theme,
    ...
    public void Print(ExceptionProfileResult? results, ProfileRenderRequest request)
    {
        ...
        ProfilerConsoleRenderHelpers.WriteProfileHeader("EXCEPTION PROFILE", request);

        var selectedType = ProfilerConsoleRenderHelpers.SelectExceptionType(results.ExceptionTypes, request.ExceptionTypeFilter);
        var filteredExceptionTypes = ProfilerConsoleRenderHelpers.FilterExceptionTypes(results.ExceptionTypes, request.ExceptionTypeFilter);
        ...
        if (summaryThrown > 0)
        {
            ...
            AnsiConsole.Write(_callTreeRenderer.BuildExceptionCallTree(new ProfilerExceptionCallTreeRequest(
                ...
                ProfilerConsoleRenderHelpers.CreateTreeRootSelectionOptions(request, resolvedRoot))));
        }
        ...
        {
            var filteredCatchSites = catchSites.Where(entry =>
                ProfilerConsoleRenderHelpers.MatchesFunctionFilter(entry.Name, request.FunctionFilter));
            if (!request.IncludeRuntime)
            {
                filteredCatchSites = filteredCatchSites.Where(entry => !CallTreeHelpers.IsRuntimeNoise(entry.Name));
            }
