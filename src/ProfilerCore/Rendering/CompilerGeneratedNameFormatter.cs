...
internal static class CompilerGeneratedNameFormatter
{
    private static readonly char[] StateMachineMethodTerminators = ['|', '>'];

    public static string? TryFormat(string typePart, string methodPart, Func<string, string> cleanTypeName)
    {
        ...
        var targetType = IsDisplayClassType(typePart)
            ? ExtractOuterType(typePart)
            : typePart;
        var prefix = string.IsNullOrWhiteSpace(targetType)
            ? string.Empty
            : cleanTypeName(targetType) + ".";
    ...
    private static string? ExtractStateMachineMethodName(string typePart)
    {
        ...
        if (localFunctionIndex >= 0)
        {
            ...
            var localEnd = typePart.IndexOfAny(StateMachineMethodTerminators, localStart);
            if (localEnd < 0)
            {
